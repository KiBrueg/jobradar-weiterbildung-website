  ...
  return (
    <Ctx.Provider value={{ toast }}>
      ...
      <div className="fixed bottom-5 right-5 z-[100] flex flex-col gap-2.5 w-[340px] max-w-[calc(100vw-2.5rem)]">
        {toasts.map((t) => (
          <div key={t.id} className="animate-slide-in flex items-start gap-3 rounded-xl border border-ink-200 bg-white px-4 py-3 shadow-lift" role="status">
            ...
            <button onClick={() => dismiss(t.id)} className="shrink-0 text-ink-400 hover:text-ink-700 transition-colors" aria-label="Schliessen"><X className="h-4 w-4" /></button>
          </div>
        ))}
      </div>
    </Ctx.Provider>
